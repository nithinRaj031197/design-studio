...
const AboutUsInfo = () => {
  ...
  const [clients, setClients] = useState<Client[]>([{ name: "", logo: null, file: null }]);
  ...
  return (
    <div className="w-full h-full p-[2rem]">
      <div className="bg-white w-full min-h-full rounded-lg">
        <div className="py-4 px-4 flex items-center justify-between flex-row">
          ...
          {editAboutPage ? (
            <div className="flex items-center gap-4">
              <Button variant="contained" className="w-[12rem] h-[3rem] !rounded-full !bg-white !text-black" onClick={cancelAboutHandler}>
                <p className="text-base font-semibold">Cancel</p>
              </Button>
              <Button variant="contained" className="w-[12rem] h-[3rem] !rounded-full !bg-[#a3865b]" onClick={updateAboutHandler}>
                <p className="text-base font-semibold">Save</p>
              </Button>
            </div>
          ) : (
            <Button variant="contained" className="w-[12rem] h-[3rem] !rounded-full !bg-[#a3865b]" onClick={editAboutHandler}>
        ...
        <div>
          {editAboutPage ? (
            <EditForm aboutContent={aboutContent} setAboutContent={setAboutContent} clients={clients} setClients={setClients} />
          ) : (
            <div className="rounded-lg shadow-lg p-6">
              <div className="w-full text-justify p-4 text-gray-700 leading-relaxed">
                ...
                <p className="text-lg">
                  Founded in 2021 and based in Gurgaon, Groyyo Design Studio is a one-stop solution for fashion and lifestyle design needs. Blending
                  creativity with innovation, the studio crafts unique, trend-forward designs that help brands stand out in the fast-paced fashion
                  industry. By merging timeless elegance with modern flair, classic aesthetics are revived with a fresh, contemporary twist. In
                  addition to offering ready-made collections, clients can book appointments for personalized customization services to bring their
                  vision to life. Whether it's original designs or bespoke solutions, Groyyo Design Studio delivers exceptional quality, style, and
                  craftsmanship to elevate any brand.
                </p>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  {logos.map((logo, index) => (
                    <div key={index} className="flex flex-col items-center">
                      <div className="text-lg font-bold text-gray-700 mb-2">{logo?.alt}</div>
                      <img
