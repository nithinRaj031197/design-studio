...
const EditForm: React.FC<EditFormProps> = ({ aboutContent, setAboutContent, clients, setClients }) => {
            ...
            <div className="flex items-center">
              {client.logo ? (
                <div>
                  ...
                  <img src={client.logo} alt="Client Logo Preview" className="w-[100px] h-auto border" />
                </div>
              ) : (
                <input type="file" accept="image/*" onChange={(e) => handleImageUpload(index, e)} className="mb-2" />
              )}
            </div>

            <Button variant="contained" color="secondary" onClick={() => handleDeleteClient(index)}>
              Delete
            </Button>
          </div>
        ))}
